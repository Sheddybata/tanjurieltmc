...
export default function DepositsPage() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      showToast(`Deposit submitted — ${res.data.paymentRequest.reference}`, 'success');
      (e.target as HTMLFormElement).reset();
      setAccountId('');
    } catch (err: unknown) {
      showToast((err as { message?: string })?.message || 'Deposit failed', 'error');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <DashboardLayout>
      <Header title="Submit Deposit" subtitle="Teller initiates — manager must approve before balance is credited" />
      <div className="p-8">
