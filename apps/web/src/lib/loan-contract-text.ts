...
export function buildLoanContractText(params: {
  ...
  const unit = ({ DAILY: 'day', WEEKLY: 'week', MONTHLY: 'month' } as Record<string, string>)[params.repaymentPlan] ?? 'period';
...
The client is desirous of becoming a member of and obtaining a loan from Tanjuriel Microcredit Limited in the amount of ${formatCurrency(params.principalAmount)} (as selected during sign-up) in ${params.tenurePeriods} installment(s) of ${formatCurrency(params.installmentAmount)} per ${unit}.
